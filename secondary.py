...
class RestauranteApp:
    ...
    def add_to_spreadsheet(self):
        ...
        try:
            df = pd.read_csv("planilha_tortas.csv", parse_dates=["Dia"])
        ...
        if not df.empty and (df["Dia"] == data_atual).any():
            # Adicionar a quantidade ao registro existente
            df.loc[df["Dia"] == data_atual, tamanho] += quantidade
        else:
            # Criar um novo registro para o dia atual
            new_row = {"Dia": data_atual, tamanho: quantidade}
            df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)

        # Ordenar o DataFrame pela coluna "Dia"
        df = df.sort_values("Dia", key=lambda x: x.astype(str)).reset_index(drop=True)
